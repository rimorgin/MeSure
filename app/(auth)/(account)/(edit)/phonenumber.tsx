...
export default function EditPhone() {
  ...
  const { userId, userContactNo, setUserContactNo } = useUserStore();
  const currentPhoneNo = userContactNo || '';
  const [contactNo, setContactNo] = useState(currentPhoneNo);
  const [loading, setLoading] = useState(false);

  const handleSubmit = async () => {
    setLoading(true)
    await setUserContactNo(userId, contactNo)
    setLoading(false)
    router.back()
  }
      ...
      <ThemedView style={styles.content}>
        ...
        <ThemedView style={styles.inputContainer}>
          ...
          >
            ...
            />
            {contactNo && (
              <TouchableOpacity onPress={() => setContactNo('')}>
        ...
        <ThemedTouchableFilled
          ...
          style={{
            marginTop: 50,
            backgroundColor: contactNo.length === 11 && contactNo !== currentPhoneNo ? mustard : '#AAA'
          }}
          disabled={!(contactNo.length === 11 && contactNo !== currentPhoneNo)}
        >
